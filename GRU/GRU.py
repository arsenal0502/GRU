...
class GRU_Model(LanguageModel):
  ...
  def add_model(self, inputs):
    ...
    for step in inputs:
      ...
      z_t=tf.sigmoid(tf.matmul(step,W_z)+tf.matmul(pre_h,U_z))
      r_t=tf.sigmoid(tf.matmul(step,W_r)+tf.matmul(pre_h,U_r))
      h_t=tf.tanh(tf.matmul(step,W)+r_t*tf.matmul(pre_h,U))
      pre_h=(tf.ones_like(z_t)-z_t)*pre_h+z_t*h_t
      gru_outputs.append(tf.nn.dropout(pre_h,self.dropout_placeholder))
...
def test_GRU():
  ...
  with tf.variable_scope('GRULM') as scope:
    model = GRU_Model(config)
    scope.reuse_variables()
    gen_model = GRU_Model(gen_config)
